...
public class MainActivity extends ActivityGPS {
    private final GPSinit gpsInit = new GPSinit(this);
	...
	FriendListDownloadTask f = new FriendListDownloadTask(getTwitter(),
		adapter);
	f.execute();
	gpsInit.setUpMap();
	 ...
	 * @SuppressLint("ShowToast")
	 * 
	 * @Override public void onClick(View v) { String location
	 * =mFplace.getText().toString(); if(location==null||location==""){
	 * Toast.makeText(getBaseContext(), getString(R.string.warning_msg2),
	 * Toast.LENGTH_LONG); return; } String map_url =
	 * "https://maps.googleapis.com/maps/api/geocode/json?"; try { //
	 * encoding special characters like space in the user input place
	 * location = URLEncoder.encode(location, "utf-8"); } catch
	 * (UnsupportedEncodingException e){ e.printStackTrace(); } String
	 * address = "address=" + location; String sensor = "sensor=false";
	 * 
	 * 
	 * // url , from where the geocoding data is fetched map_url = map_url +
	 * address + "&" + sensor; // String modifiedURL=
	 * url.toString().replace(" ", "%20");
	 * 
	 * // Instantiating DownloadTask to get places from Google Geocoding
	 * service // in a non-ui thread DownloadTask downloadTask = new
	...
	if (!gpsInit.setUpLocation()) {
	    Toast.makeText(this, getString(R.string.warning_msg1),
		    Toast.LENGTH_LONG);
	} else {
	    // CameraUpdate
	    // cameraUpdate=CameraUpdateFactory.newLatLngZoom(fastGPSloc(), 10);
	...
	parent.setOnItemClickListener(new OnItemClickListener() {
	    ...
	    public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
		    long arg3) {
		FriendListAdapter fAdapter = (FriendListAdapter) arg0
			.getAdapter();
		FriendListItem row = (FriendListItem) fAdapter.getItem(arg2);
